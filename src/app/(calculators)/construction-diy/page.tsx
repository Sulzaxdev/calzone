...
import { ChevronRight, Hammer, Ruler, Layers, HardHat, Warehouse, Construction, Zap, Info } from "lucide-react";
import { categories } from "@/lib/calculators";
import { BreadcrumbSchema } from "@/components/seo/breadcrumb-schema";

export const metadata = {
    title: "Construction & DIY Calculators | Structural Project Intelligence",
    description: "Plan your home improvement projects with precision. Our tactical construction calculators handle concrete volumes, brickwork, decking materials, and tarmac resurfacing costs.",
};

export default function ConstructionLandingPage() {
    const category = categories.find(c => c.title === "Construction & DIY");
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 items-center">
                        <div className="animate-in slide-in-from-left duration-700">
                            ...
                            <p className="text-xl text-white/90 mb-10 leading-relaxed max-w-xl drop-shadow-lg">
                                Engineer your DIY success. From concrete bag counts to tarmac surfacing estimates, our tactical construction engine provides the material precision needed for professional results.
                            </p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-5 rounded-2xl group hover:bg-white/20 transition-all">
                                    ...
                                    <h3 className="text-white font-bold text-sm mb-1">Material Accuracy</h3>
                                    <p className="text-white/60 text-xs leading-relaxed">
                                        Calculate exact volumes for concrete, mortar, and timber to minimize waste and cost.
                                    </p>
                                </div>
                                <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-5 rounded-2xl group hover:bg-white/20 transition-all">
                                    <Warehouse className="w-8 h-8 text-orange-400 mb-3" />
                                    <h3 className="text-white font-bold text-sm mb-1">Site Optimization</h3>
                                    <p className="text-white/60 text-xs leading-relaxed">
                                        Tactical planning for garden walls, driveways, and flooring foundations.
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="relative bg-white/5 backdrop-blur-3xl border border-white/10 p-10 rounded-[2.5rem] shadow-2xl overflow-hidden">
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-white font-bold text-lg">Structural Data</h4>
                                            <p className="text-white/50 text-xs">Load & Volume calibration</p>
                                        </div>
                                    </div>
                                    <Construction className="w-6 h-6 text-white/30 animate-pulse" />
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs font-bold text-white/70 uppercase">
                                            ...
                                            <span className="text-primary">15% Efficiency Gain</span>
                                        </div>
                                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full bg-primary w-[80%] rounded-full shadow-[0_0_15px_rgba(var(--primary),0.5)]"></div>
                                        </div>
                                    </div>
                                    <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20">
                                        <span className="text-[10px] font-bold text-primary uppercase tracking-widest block mb-1">Builder Insight</span>
                                        <p className="text-white/80 text-xs leading-relaxed italic">
                                            "A 10% overflow is standard for concrete pours, but our calculator ensures your base measurement is structurally sound before you order."
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="container mx-auto px-4 max-w-7xl -mt-10 relative z-20">
                <div className="mb-12 flex items-center gap-4">
                    ...
                    <h2 className="text-3xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tighter">
                        Building & DIY Tools <span className="text-primary tracking-widest text-sm align-middle ml-2">[{category.calculators.length}]</span>
                    </h2>
                </div>
                        ...
                        <Link key={calc.name} href={calc.href} className="group">
                            <Card className="h-full p-8 bg-white/80 dark:bg-card/50 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 group-hover:border-primary/50 flex flex-col">
                                <div className="mb-6 w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform duration-300">
                                    {calc.name.toLowerCase().includes('concrete') || calc.name.toLowerCase().includes('brick') ? <Layers className="w-7 h-7" /> : <Hammer className="w-7 h-7" />}
                                </div>
                ...
                <div className="mt-16 bg-linear-to-br from-primary/10 via-transparent to-primary/5 border border-primary/20 p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden group">
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">Technical Standards</h3>
                            <p className="text-slate-600 dark:text-slate-400 max-w-2xl leading-relaxed">
                                Our construction calculators are rigorously tested for UK standard material dimensions, from metric brick sizes to standard tarmac coverage rates. CalZone provides the structural intelligence required for small-scale DIY and professional building projects alike.
                            </p>
                        </div>
                    </div>
