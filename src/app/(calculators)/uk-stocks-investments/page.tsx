...
export const metadata = {
    ...
    description: "Master your investments with our elite suite of UK stock market calculators. Precision tools for Capital Gains Tax (CGT), portfolio growth, and dividend analytics.",
};

export default function UKStockMarketLandingPage() {
    const category = categories.find(c => c.title === "UK Stock Market & Investments");
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950/10 pb-20 overflow-x-hidden pt-12">
            ...
            <section className="relative w-full min-h-[600px] flex items-center pt-24 pb-16 overflow-hidden">
                ...
                <div className="absolute inset-0 z-0">
                    <Image
                        src="/uk_stock_market_hero.png"
                        alt="UK Stock Market Intelligence"
                        fill
                        priority
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="animate-in slide-in-from-left duration-700">
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs font-bold mb-6 tracking-widest uppercase">
                                ...
                                Investment Intelligence v1.0
                            </div>
                            <h1 className="text-5xl md:text-7xl font-black text-white mb-6 leading-[1.1] tracking-tight drop-shadow-2xl">
                                UK Stock <br />
                                <span className="text-primary italic">Wealth Engine.</span>
                            </h1>
                            <p className="text-xl text-white/90 mb-10 leading-relaxed max-w-xl drop-shadow-lg">
                                Navigate the London Stock Exchange with precision. Our tactical calculators handle HMRC tax logic, compounding growth, and portfolio risk with institutional accuracy.
                            </p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-5 rounded-2xl group hover:bg-white/20 transition-all">
                                    <PieChart className="w-8 h-8 text-primary mb-3" />
                                    <h3 className="text-white font-bold text-sm mb-1 line-clamp-1">Tax Optimization</h3>
                                    <p className="text-white/60 text-xs leading-relaxed">
                                        Minimize your CGT liability with real-time HMRC allowance calibration and loss harvesting.
                                    </p>
                                </div>
                                <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-5 rounded-2xl group hover:bg-white/20 transition-all">
                                    <Landmark className="w-8 h-8 text-amber-400 mb-3" />
                                    <h3 className="text-white font-bold text-sm mb-1 line-clamp-1">LSE Standards</h3>
                                    <p className="text-white/60 text-xs leading-relaxed">
                                        Engineered for UK shares, ETFs, and trust structures following Section 104 pooling rules.
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="relative bg-white/5 backdrop-blur-3xl border border-white/10 p-10 rounded-[2.5rem] shadow-2xl overflow-hidden">
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-white font-bold text-lg">Portfolio Analytics</h4>
                                            <p className="text-white/50 text-xs">Real-time yield assessment</p>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs font-bold text-white/70 uppercase">
                                            <span>Annual ISA Utilization</span>
                                            <span className="text-primary">£20,000 Cap.</span>
                                        </div>
                                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full bg-primary w-[85%] rounded-full shadow-[0_0_15px_rgba(var(--primary),0.5)]"></div>
                                        </div>
                                    </div>
                                    <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20">
                                        <span className="text-[10px] font-bold text-primary uppercase tracking-widest block mb-1">Market Insight</span>
                                        <p className="text-white/80 text-xs leading-relaxed italic">
                                            "Utilizing your annual CGT allowance effectively can enhance net portfolio returns by up to 2.4% annually for basic rate taxpayers."
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="container mx-auto px-4 max-w-7xl -mt-10 relative z-20">
                ...
                <div className="mb-12 flex items-center gap-4">
                    ...
                    <h2 className="text-3xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tighter">
                        Investment Calculators <span className="text-primary tracking-widest text-sm align-middle ml-2">[{category.calculators.length}]</span>
                    </h2>
                </div>
                        ...
                        <Link key={calc.name} href={calc.href} className="group">
                            <Card className="h-full p-8 bg-white/80 dark:bg-card/50 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 group-hover:border-primary/50 flex flex-col">
                                <div className="mb-6 w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform duration-300">
                                    {calc.name.toLowerCase().includes('tax') || calc.name.toLowerCase().includes('gains') ? <Receipt className="w-7 h-7" /> : <TrendingUp className="w-7 h-7" />}
                                </div>
                ...
                <div className="mt-16 bg-linear-to-br from-primary/10 via-transparent to-primary/5 border border-primary/20 p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden group">
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">HMRC Compliance & Precision</h3>
                            <p className="text-slate-600 dark:text-slate-400 max-w-2xl leading-relaxed">
                                Our investment engine is rigorously calibrated with the latest UK tax year allowances and HMRC share pooling rules. From the 30-day "bed and breakfast" rule to Section 104 pooling, CalZone provides the technical depth required for professional-grade financial planning.
                            </p>
                        </div>
                    </div>
                    {/* Decorative Blob */}
